...
def train(epoch,net,net2,optimizer,labeled_trainloader,unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x) in enumerate(labeled_trainloader):      
        ...
        logits_adv, feat_adv = net(inputs_adv, adv=True, return_features=True)
        prob_adv = torch.softmax(logits_adv, dim=-1)
        y_adv = torch.log(torch.gather(prob_adv, 1, targets_uadv.view(-1, 1)).squeeze(dim=1))
        Ladv = torch.mean((prob_adv - targets_ori)**2)
        pip = (normalize_flatten_features(feat_adv) - \
        ...
        wandb.log({'loss/l_adv': Ladv.data.item(),
                 ...
                 'group1/y_w': y_w.mean().cpu().detach().numpy(),
                 'group1/pip': pip.mean().cpu().detach().numpy(),
                 'group1/l2_norm': torch.mean((inputs_adv - inputs_ori).reshape(
                     inputs_ori.shape[0], -1).norm(dim=1)),
                 'group1/l2_norm_his': wandb.Histogram((inputs_adv - inputs_ori).reshape(
                     inputs_ori.shape[0], -1).norm(dim=1).cpu().detach().numpy(), num_bins=512)})
...
stats_log=open('./checkpoint/adv%s_%.1f_%s'%(args.dataset,args.r,args.noise_mode)+'_stats.txt','w')
test_log=open('./checkpoint/adv%s_%.1f_%s'%(args.dataset,args.r,args.noise_mode)+'_acc.txt','w')
