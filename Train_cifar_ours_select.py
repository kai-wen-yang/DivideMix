...
def train(epoch,net,net2,optimizer,labeled_trainloader,unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x) in enumerate(labeled_trainloader):      
        ...
        Ladv = torch.mean((prob_adv - mixed_targetadv[batch_size*2:])**2, dim=1)
        pip = (normalize_flatten_features(feat_adv) - \
        ...
        loss = Lx + lamb * (Lu+Ladv[mask].mean()) + penalty
        ...
        sys.stdout.flush()
        wandb.log({'loss/l_adv':Ladv.mean().cpu().detach().numpy(),
        ...
        if mask.sum()>0:
            wandb.log({'loss/L_advselect':Ladv[mask].mean().cpu().detach().numpy()}, commit=False)
